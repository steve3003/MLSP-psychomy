...
trainSet = read.arff("Train/data_sel.arff")
#labels = read.csv("Train/train_labels.csv")
#trainSet = merge(trainSet, labels)
#trainSet = trainSet[-1]

testSet = merge(FNC_test, SBM_test)
testSet = testSet[names(trainSet[-15])]

#Creating naive bayes classifier
NBclass = naiveBayes(trainSet[-15], factor(trainSet$Class))
...
write.csv(example,file='GNbayes_submission_sel.csv',row.names=FALSE)
